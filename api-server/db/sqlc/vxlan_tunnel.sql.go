...
RETURNING id, name, tag, tunnel_ip, local_ip, remote_ip, remote_mac, status, created_at
`
...
SELECT id, name, tag, tunnel_ip, local_ip, remote_ip, remote_mac, status, created_at FROM vxlan_tunnel
WHERE id = $1 LIMIT 1
`
...
const updateVxlanStatus = `-- name: UpdateVxlanStatus :exec
UPDATE vxlan_tunnel 
SET status=$2
WHERE id=$1
`

type UpdateVxlanStatusParams struct {
	ID     int64  `json:"id"`
	Status string `json:"status"`
}

func (q *Queries) UpdateVxlanStatus(ctx context.Context, arg UpdateVxlanStatusParams) error {
	_, err := q.db.ExecContext(ctx, updateVxlanStatus, arg.ID, arg.Status)
	return err
}
